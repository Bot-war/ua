...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['11', '9.0', '10'])
            self.android_model = random.choice(['Nokia C3'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {} Build/QP1A.190711.020; en-in) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Mobile Safari/537.36'.format(self.android_version, self.android_model, self.build, self.browser_version))
